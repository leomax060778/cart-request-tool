...
function confirmToken(token, userId) {
    ...
    if (token) {
        ...
        //get token sent for this user
        var userRecovery = dbLogin.getTokenRecovery(token);
        if (!userRecovery) {
            throw ErrorLib.getErrors().LoginError("", "", "Invalid credentials. The Token doesn't exists");
        }
        ...
        throw ErrorLib.getErrors().LoginError("", "", "Invalid credentials. User doesn't exists");
    }
}
...
function validateCurrentPassword(username, password) {
    var currentUser = userLib.getUserByUserName(username);
    if (!currentUser) {
        throw ErrorLib.getErrors().LoginError("", "", "Invalid credentials. Please log in again");
    }
    ...
    if (userHashedPassword && currentUserPassword && userHashedPassword != currentUserPassword) {
        throw ErrorLib.getErrors().LoginError("", "", "Invalid Current Password.");
    }
